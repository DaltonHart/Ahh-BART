...
class Config {
    ...
    async load() {
        ...
        this.arch = (os_1.default.arch() === 'ia32' ? 'x86' : os_1.default.arch());
        ...
        this.home = process.env.HOME || (this.windows && this.windowsHome()) || os_1.default.homedir() || os_1.default.tmpdir();
        this.cacheDir = this.scopedEnvVar('CACHE_DIR') || this.macosCacheDir() || this.dir('cache');
        this.configDir = this.scopedEnvVar('CONFIG_DIR') || this.dir('config');
        this.dataDir = this.scopedEnvVar('DATA_DIR') || this.dir('data');
        this.errlog = path_1.default.join(this.cacheDir, 'error.log');
    ...
    async loadUserPlugins() {
        if (this.options.userPlugins !== false) {
            try {
                const userPJSONPath = path_1.default.join(this.dataDir, 'package.json');
    ...
    s3Url(key) {
        ...
        url.pathname = path_1.default.join(url.pathname, key);
        return url.toString();
    }
    dir(category) {
        const base = process.env[`XDG_${category.toUpperCase()}_HOME`]
            || (this.windows && process.env.LOCALAPPDATA)
            || path_1.default.join(this.home, category === 'data' ? '.local/share' : '.' + category);
        return path_1.default.join(base, this.dirname);
    }
    windowsHome() { return this.windowsHomedriveHome() || this.windowsUserprofileHome(); }
    windowsHomedriveHome() { return (process.env.HOMEDRIVE && process.env.HOMEPATH && path_1.default.join(process.env.HOMEDRIVE, process.env.HOMEPATH)); }
    windowsUserprofileHome() { return process.env.USERPROFILE; }
    macosCacheDir() { return this.platform === 'darwin' && path_1.default.join(this.home, 'Library', 'Caches', this.dirname) || undefined; }
    _shell() {
        let shellPath;
