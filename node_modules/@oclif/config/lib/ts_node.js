...
function registerTSNode(root) {
    ...
    if (tsconfig.compilerOptions.rootDirs) {
        rootDirs.push(...tsconfig.compilerOptions.rootDirs.map(r => path_1.default.join(root, r)));
    }
    else {
...
function tsPath(root, orig) {
    if (!orig)
        return orig;
    orig = path_1.default.join(root, orig);
    try {
        registerTSNode(root);
        ...
        const lib = path_1.default.join(root, outDir); // ./lib
        const src = path_1.default.join(root, rootDir); // ./src
        const relative = path_1.default.relative(lib, orig); // ./commands
        const out = path_1.default.join(src, relative); // ./src/commands
        ...
        if (fs_1.default.existsSync(out) || fs_1.default.existsSync(out + '.ts'))
            return out;
        else
            return orig;
    }
